...
EFF_RPM_LIMIT = 4700.0    # RPM — governor limit / pulse stop (most efficient RPM)
...
def load_and_preprocess_track(filename):
    ...
    # v = (RPM * 2 * pi * R) / (G * 60)
    v_pulse_stop_eff = (EFF_RPM_LIMIT * 2 * np.pi * engine_model.WHEEL_RADIUS) / (engine_model.GEAR_RATIO * 60)
    v_pulse_stop_kmh = v_pulse_stop_eff * 3.6
...
def evaluate_strategy(distance_m, slope, is_curve, v_avg_target=25.05, v_pulse_stop=35.0, num_laps=4):
    ...
    # v = (RPM * 2 * pi * R) / (G * 60)
    v_pulse_stop_mps = (EFF_RPM_LIMIT * 2 * np.pi * R) / (G * 60)  # ~34.5 km/h at 4700 RPM
...
<body>
    ...
    try:
        ...
        plt.axis('equal')
        v_pulse_stop_kmh = (EFF_RPM_LIMIT * 2 * np.pi * engine_model.WHEEL_RADIUS) / (engine_model.GEAR_RATIO * 60) * 3.6
